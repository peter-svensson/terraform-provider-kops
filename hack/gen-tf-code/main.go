...
func main() {
	...
	build(
		...
		"docs/resources/",
		parser,
		generate(resources.DeleteOptions{}),
		...
		generate(kops.NodeTerminationHandlerSpec{},
			...
			nullable("EnableScheduledEventDraining", "EnableSQSTerminationDraining", "ExcludeFromLoadBalancers"),
		),
		...
		generate(kops.ClusterAutoscalerConfig{},
			...
			nullable("CreatePriorityExpenderConfig", "SkipNodesWithCustomControllerPods"),
		),
		...
		generate(kops.KubeAPIServerConfig{},
			nullable("AnonymousAuth", "AuditWebhookBatchThrottleEnable"),
		),
		...
		generate(kops.CiliumIngressSpec{},
			nullable("EnableSecretsSync", "EnforceHttps"),
		),
		generate(kops.CiliumNetworkingSpec{},
			required("PreallocateBPFMaps", "EnableRemoteNodeIdentity"),
			nullable("CniExclusive", "EnableL7Proxy", "EnableEndpointHealthChecking", "InstallIptablesRules"),
		),
		generate(kops.HubbleSpec{}),
		generate(kops.LyftVPCNetworkingSpec{}),
		generate(kops.GCPNetworkingSpec{}),
		...
		// 1.31
		generate(kops.ClusterField{}),
		generate(corev1.EnvVar{},
			required("Name"),
		),
		generate(corev1.EnvVarSource{}),
		generate(corev1.ObjectFieldSelector{},
			required("FieldPath"),
		),
		generate(corev1.ResourceFieldSelector{},
			required("Resource"),
			exclude("Divisor"),
		),
		generate(corev1.ConfigMapKeySelector{},
			required("Key"),
		),
		generate(corev1.LocalObjectReference{}),
		generate(corev1.SecretKeySelector{},
			required("Key"),
		),
		// 1.32
		generate(kops.CiliumGatewayAPISpec{},
			nullable("EnableSecretsSync"),
		),
	...
	build(
		...
		generate(config.Scaleway{},
			sensitive("AccessKey", "SecretKey"),
		),
