...
- `storage_account_id` - (Computed) - String - StorageAccountID specifies the storage account used for the cluster installation.
...
- `node_selector_terms` - (Computed) - List([node_selector_term](#node_selector_term)) - Required. A list of node selector terms. The terms are ORed.<br />+listType=atomic.
...
- `match_expressions` - (Computed) - List([node_selector_requirement](#node_selector_requirement)) - A list of node selector requirements by node's labels.<br />+optional<br />+listType=atomic.
- `match_fields` - (Computed) - List([node_selector_requirement](#node_selector_requirement)) - A list of node selector requirements by node's fields.<br />+optional<br />+listType=atomic.
...
- `match_expressions` - (Computed) - List([label_selector_requirement](#label_selector_requirement)) - matchExpressions is a list of label selector requirements. The requirements are ANDed.<br />+optional<br />+listType=atomic.
...
- `values` - (Computed) - List(String) - values is an array of string values. If the operator is In or NotIn,<br />the values array must be non-empty. If the operator is Exists or DoesNotExist,<br />the values array must be empty. This array is replaced during a strategic<br />merge patch.<br />+optional<br />+listType=atomic.
...
- `compaction_interval` - (Computed) - Duration - CompactionInterval is an interval of requesting compaction from apiserver.<br />If the value is 0, no compaction will be issued.
...
- `authentication_config_file` - (Computed) - String - AuthenticationConfigFile is the location of the authentication-config<br />this option is mutually exclusive with all OIDC options.
...
- `watch_cache` - (Computed) - Bool - Used to disable watch caching in the apiserver, defaults to enabling caching by omission.
- `watch_cache_sizes` - (Computed) - List(String) - Set the watch-cache-sizes parameter for the apiserver<br />The only meaningful value is setting to 0, which disable caches for specific object types.<br />Setting any values other than 0 for a resource will yield no effect since the caches are dynamic.
...
- `kube_api_qps` - (Computed) - Int - KubeAPIQPS Burst to use while talking with kubernetes apiserver. (default 50).
...
- `kube_router` - (Computed) - [kuberouter_networking_spec](#kuberouter_networking_spec)
...
- `aws_src_dst_check` - (Computed) - String - AWSSrcDstCheck enables/disables ENI source/destination checks (AWS IPv4 only)<br />Options: Disable (default for IPv4), Enable, or DoNothing.
...
- `network_policy_agent_image` - (Computed) - String - NetworkPolicyAgentImage is the container image to use for the network policy agent.
...
- `registry` - (Computed) - String - Registry overrides the default Cilium container registry (quay.io).
...
- `node_encryption` - (Computed) - Bool - NodeEncryption enables encryption for pure node to node traffic.<br />Default: false.
...
- `cluster_id` - (Computed) - Int - ClusterID is the ID of the cluster. It is only relevant when building a mesh of clusters.<br />Must be a number between 1 and 255.
...
- `oidc` - (Computed) - [oidc_authentication_spec](#oidc_authentication_spec)
...
- `image` - (Computed) - String - Image is the AWS IAM Authenticator container image to use.
...
- `username_claim` - (Computed) - String - UsernameClaim is the OpenID claim to use as the username.<br />Note that claims other than the default ('sub') are not guaranteed to be<br />unique and immutable.
- `username_prefix` - (Computed) - String - UsernamePrefix is the prefix prepended to username claims to prevent<br />clashes with existing names (such as 'system:' users).
- `groups_claims` - (Computed) - List(String) - GroupsClaims are the names of the custom OpenID Connect claims for<br />specifying user groups (optional).
- `groups_prefix` - (Computed) - String - GroupsPrefix is the prefix prepended to group claims to prevent<br />clashes with existing names (such as 'system:' groups).
- `issuer_url` - (Computed) - String - IssuerURL is the URL of the OpenID issuer. Only the HTTPS scheme will<br />be accepted.<br />If set, will be used to verify the OIDC JSON Web Token (JWT).
- `client_id` - (Computed) - String - ClientID is the client ID for the OpenID Connect client. Must be set<br />if issuerURL is set.
- `required_claims` - (Computed) - Map(String) - RequiredClaims are key/value pairs that describe required claims in the ID Token.<br />If set, the claims are verified to be present in the ID Token with corresponding values.
...
- `image` - (Computed) - String - Image is the container image.
...
- `container_registry` - (Computed) - String - ContainerRegistry is a url for to a container registry.
- `file_repository` - (Computed) - String - FileRepository is the url for a private file serving repository.
- `container_proxy` - (Computed) - String - ContainerProxy is a url for a pull-through proxy of a container registry.
...
- `balance_similar_node_groups` - (Computed) - Bool - BalanceSimilarNodeGroups makes the cluster autoscaler treat similar node groups as one.<br />Default: false.
- `emit_per_nodegroup_metrics` - (Computed) - Bool - EmitPerNodegroupMetrics If true, publishes the node groups min and max metrics count set on the cluster autoscaler.<br />Default: false.
