...
- `cpu_request` - (Optional) - Quantity - CPURequest, cpu request compute resource for AWS Load Balancer Controller.<br />Default: 100m.
- `cpu_limit` - (Optional) - Quantity - CPULimit, cpu limit compute resource for AWS Load Balancer Controller.<br />Default: 200m.
- `memory_request` - (Optional) - Quantity - MemoryRequest, memory request compute resource for AWS Load Balancer Controller.<br />Default: 200Mi.
- `memory_limit` - (Optional) - Quantity - MemoryLimit, memory limit compute resource for AWS Load Balancer Controller.<br />Default: 500Mi.
...
- `lifecycle_hook_timeout` - (Optional) - Int - LifecycleHookTimeout is the timeout for the ASG lifecycle hook in seconds.
- `additional_images` - (Optional) - List(String) - AdditionalImages is a list of additional container images to pull into the warm pool instances.
...
- `version` - (Optional) - String - Version is the container image tag used.<br />Default: The latest stable release which is compatible with your Kubernetes version.
- `default_storage_class_name` - (Optional) - String - Default StorageClassName is the name of the default StorageClass created for GCP PD CSI driver.
...
- `use_ecr_credentials_for_mirrors` - (Optional) - Bool - Enables Kubelet ECR Credential helper to pass credentials to containerd mirrors, to use ECR as a pull-through cache.
- `install_cri_ctl` - (Optional) - Bool - InstallCriCtl installs crictl (default "false").
- `install_nerd_ctl` - (Optional) - Bool - InstallNerdCtl installs nerdctl (default "false").
- `sandbox_image` - (Optional) - String - SandboxImage is the image used for the sandbox container.
...
- `driver_package` - (Optional) - String - Package is the name of the nvidia driver package that will be installed.<br />Default is "nvidia-driver-535-server".
- `enabled` - (Optional) - Bool - Enabled determines if kOps will install the Nvidia GPU runtime and drivers.<br />They will only be installed on instances that has an Nvidia GPU.
- `device_plugin_image` - (Optional) - String - Image defines the container image used to deploy the Nvidia Kubernetes Device Plugin.
...
- `pod_annotations` - (Optional) - Map(String) - PodAnnotations makes possible to add additional annotations to CoreDNS Pods.<br />Default: none.
...
- `audit_webhook_batch_throttle_enable` - (Optional) - Bool([Nullable](#nullable-arguments)) - AuditWebhookBatchThrottleEnable is Whether batching throttling is enabled. Only used in batch mode. (default true).
...
- `delete_collection_workers` - (Optional) - Int - DeleteCollectionWorkers indicates the number of workers spawned for DeleteCollection call. These are used to speed up namespace cleanup.
...
- `file_key_ref` - (Optional) - [file_key_selector](#file_key_selector) - FileKeyRef selects a key of the env file.<br />Requires the EnvFiles feature gate to be enabled.<br /><br />+featureGate=EnvFiles<br />+optional.
...
- `volume_name` - (Optional) - String - The name of the volume mount containing the env file.<br />+required.
- `path` - (Optional) - String - The path within the volume from which to select the file.<br />Must be relative and may not contain the '..' path or start with '..'.<br />+required.
...
- `pod_infra_container_image` - (Optional) - String - PodInfraContainerImage is the image whose network/ipc containers in each pod will use.<br />DEPRECATED: Image garbage collector will get sandbox image information from CRI.
...
- `cpu_limit` - (Optional) - Quantity - CPULimit of NodeProblemDetector container.
...
- `enable_l7_proxy` - (Optional) - Bool([Nullable](#nullable-arguments)) - EnableL7Proxy enables L7 proxy for L7 policy enforcement.<br />Default: true.
...
- `install_iptables_rules` - (Optional) - Bool([Nullable](#nullable-arguments)) - InstallIptablesRules enables installing the base IPTables rules used for masquerading and kube-proxy.<br />Default: true.
...
- `cni_exclusive` - (Optional) - Bool([Nullable](#nullable-arguments)) - CniExclusive configures whether to remove other CNI configuration files.<br />Default: true.
...
- `enforce_https` - (Optional) - Bool([Nullable](#nullable-arguments)) - EnforceHttps specifies whether HTTPS enforcement is enabled for Ingress traffic.<br />Default: true.
- `enable_secrets_sync` - (Optional) - Bool([Nullable](#nullable-arguments)) - EnableSecretsSync specifies whether synchronization of secrets is enabled.<br />Default: true.
...
- `cpu_limit` - (Optional) - Quantity - CPULimit CPU limit of AWS IAM Authenticator container.
...
- `skip_nodes_with_custom_controller_pods` - (Optional) - Bool([Nullable](#nullable-arguments)) - SkipNodesWithCustomControllerPods makes the cluster autoscaler skip scale-down of nodes with pods owned by custom controllers.<br />Default: true.
...
- `create_priority_expender_config` - (Optional) - Bool([Nullable](#nullable-arguments)) - CreatePriorityExpenderConfig makes kOps create the priority-expander ConfigMap<br />Default: true.
...
- `discovery_service` - (Optional) - [discovery_service_options](#discovery_service_options) - DiscoveryService configures discovery using a hosted discovery service.
...
- `url` - (Optional) - String - URL is the base URL of the discovery service, including universe ID if applicable.
