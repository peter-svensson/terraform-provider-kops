...
func TestExpandResourceKubeletConfigSpec(t *testing.T) {
	...
	}{
		{
			...
			args: args{
				in: map[string]interface{}{
					...
					"pod_pids_limit":                           nil,
					"experimental_allocatable_ignore_eviction": nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := ExpandResourceKubeletConfigSpec(tt.args.in)
			if diff := cmp.Diff(tt.want, got); diff != "" {
				t.Errorf("ExpandResourceKubeletConfigSpec() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
...
func TestFlattenResourceKubeletConfigSpecInto(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "SeccompDefault - default",
			args: args{
				in: func() kops.KubeletConfigSpec {
					subject := kops.KubeletConfigSpec{}
					subject.SeccompDefault = nil
					return subject
				}(),
			},
			want: _default,
		...
		{
			name: "ExperimentalAllocatableIgnoreEviction - default",
			args: args{
				in: func() kops.KubeletConfigSpec {
					subject := kops.KubeletConfigSpec{}
					subject.ExperimentalAllocatableIgnoreEviction = nil
					return subject
				}(),
			},
			want: _default,
		...
		{
			name: "MemorySwapBehavior - default",
			args: args{
				in: func() kops.KubeletConfigSpec {
					subject := kops.KubeletConfigSpec{}
